...
초대 링크 `https://onmatout.com/a/OMS-XXXX` 를 동작시키기 위해 **onmatout.com 도메인**에 올려야 하는 파일들입니다. (앱 코드가 아니라 도메인/호스팅에 배치)
...
- `/a/OMS-EX7G` 같은 하위 경로가 모두 `a/index.html` 로 가도록 rewrite (예: Vercel `rewrites`, Nginx `try_files`, Netlify `_redirects`).
...
## 5. 검증
- iOS: 빌드 설치 후 메모/메시지에서 `https://onmatout.com/a/OMS-XXXX` 길게 눌러 "온매트아웃에서 열기" 뜨는지.
