# ONMATOUT Pivot PRD v0.7

## 요가 선생님과 회원을 수업 밖에서도 연결하는 AI 클래스 케어 앱

문서 버전: v0.7
작성일: 2026-07-03
이전 버전: v0.6 (2026-06-09), v0.5 (2026-06-08), v0.4 (2026-06-05)
프로젝트명: ONMATOUT
플랫폼: Mobile App
...
v0.6 → v0.7 은 v0.6 이후 추가 구현을 PRD에 동기화하는 업데이트. (작성일 2026-07-03, 앱 버전 2.0.4)

### 역할 구조 통합 + 네비게이터 정합성

- 별도 "원장" 역할을 두지 않고 **수련생 / 선생님** 2역할만 유지 (추천 구조). 선생님 스튜디오 스위처에 **내가 만든 요가원(원장) + 합류한 요가원(선생님)** 을 함께 노출하고 행별 **원장/선생님 뱃지** 표시
- **RoleNavigatorGuard** (영속 컴포넌트) 도입 — 로그인 직후 역할이 늦게 확정돼 활성 역할과 루트 네비게이터가 어긋나던 문제(선생님인데 수련생 화면)를 감시하고 교정
- 역할 전환 시 랜딩 탭 규칙: 로그인 사용자는 **클래스 탭**, 비로그인(게스트)은 로그인 필요 없는 **아사나 탭**. `navigation/roleRoot.ts` 로 teacher/student/guest 루트 단일화

### 선생님 탭 재구성 (수련생 탭 → 클래스 탭 세그먼트, 아사나 탭 추가)

- 선생님도 아사나 사전이 필요해 **아사나 탭 추가**. 하단 탭 5개 한계를 지키기 위해 **수련생 탭을 제거**하고 클래스 탭 상단 **[수업 | 수련생] 세그먼트**로 통합
- 결과적으로 수련생/선생님 탭이 **클래스 / 아사나 / 요가톡 / 시퀀스 / 수련** 으로 대칭. 공용 `SegmentedControl` 컴포넌트 신설, `ClassesBody` / `MembersBody` 로 본문 분리(공용 헤더 공유)

### 시퀀스: 수련생도 생성 가능

- 시퀀스 생성/수정/삭제 RLS가 이미 **소유자(`teacher_id = auth.uid()`) 기준**이라 DB 변경 없이 수련생에게 개방
- 수련생 시퀀스 탭에 **"새 시퀀스" FAB** (게스트는 로그인 유도). 생성 화면(`TeacherRoutineCreate`)을 `origin` 파라미터로 재사용 → 발행 후 수련생은 수련생 상세로 이동
- 공개/비공개 선택(**기본 공개**). "내 시퀀스" 탭 = 공유받은 것 + 내가 만든 것. 본인 시퀀스는 상세 헤더 "⋯" 메뉴에서 수정/삭제
- 시퀀스 카드 작성자 표기 우선순위: **요가원명 → 작성자 닉네임 → 폴백**. (수련생 시퀀스가 "선생님"으로 잘못 뜨던 문제 수정)

### 시퀀스: 텍스트 붙여넣기 등록

- 아사나 이름을 **한 줄에 하나씩 붙여넣기** → 별칭/한글/영문/산스크리트 기반 자동 매칭(임계값) → **검수 시트**에서 후보 확인 후 선택 → 시퀀스에 추가
- 생성 화면에 제목 입력 인지성 개선(pill) + 공개/비공개 토글(기본 공개)

### 시퀀스: 스토리 공유 + 공유 딥링크

- **스토리 카드**: 9:16 다크 카드(스네이크 그리드, 제목 중심)를 이미지로 캡처해 공유 + **복사 가능한 딥링크** 노출(스토리 링크 스티커/카톡 붙여넣기용)
- **공유 딥링크** `onmatout.com/r/{routineId}`:
  - 웹 랜딩(onmatout_home)이 서버에서 **OG 미리보기(제목 카드, next/og)** 생성 → 카카오톡/SNS에서 리치 프리뷰
  - 카카오톡 인앱 브라우저 대응: **설치 앱 우선 열기** (iOS `onmatout://` 스킴 + 타이머 스토어 폴백, Android `intent://` 자체 폴백)
  - 앱은 스킴/유니버설/앱 링크로 열려 해당 **시퀀스 상세로 이동** (cold/warm start 모두 처리)
- 시퀀스 상세(선생님/수련생)에서 **카테고리 뱃지 제거**

### 요가톡: 메시지 수정/삭제 + 즉시 로딩

- 1:1 및 그룹 모두 메시지 **수정/삭제(완전 삭제)** 지원. 롱프레스 → 수정/삭제. **인라인 수정**(입력창에 원문 로드 + "메시지 수정" 배너, 카카오톡식), `(수정됨)` 표시
- 요가톡 목록 **즉시 표시**: AsyncStorage 캐시 하이드레이션으로 진입 즉시 직전 목록 노출, 백그라운드 갱신 (로딩바 체감 제거)
- 그룹 채팅 UI: 발신자 아바타 + 선생님 역할 칩 + 내/상대 버블 좌우 구분. 1:1 헤더에 상대 이름 + (상대가 선생님일 때만) 역할 칩

### 수련생 상태 변경 이력

- `student_profiles` 상태/커스텀 변경 + `memberships` 일시정지/재개를 **트리거로 자동 기록**
- 회원 상세 "상태 내역 보기" → **타임라인 화면**(수련 시작/휴식/재개, 수업권 일시정지/재개, 일시정지 기간 표시)

### Auth: 가입 시 닉네임 자동 생성

- 가입 시 **요가 테마 랜덤 닉네임 자동 생성**(형용사 + 자연 + 동물/아사나명 조합, 중복 시 재시도 후 숫자 폴백). 수동 닉네임 입력 화면 제거, 온보딩에서도 닉네임 입력하지 않음

### 데이터 모델 / 인프라 변경 (v0.7)

- `student_status_history`, `membership_status_history` 신규 + 상태 변경 트리거(SECURITY DEFINER)
- `routines` 에 **익명(anon) SELECT 정책** 추가 (`visibility='public'` 한정) — 웹 시퀀스 공유 랜딩의 OG 제목 조회용
- (DB 변경 아님) 시퀀스 생성/수정/삭제, 항목, 공유 RLS가 모두 `routines.teacher_id = auth.uid()` 소유자 기준이라 수련생 생성 개방에 스키마 변경 불필요
- 웹(onmatout_home): `/r/{id}` 시퀀스 공유 랜딩(서버 OG + next/og + 앱 우선 열기), AASA `/r/*`, Android App Link `/r`, iOS `onmatout` 스킴 + `applinks:onmatout.com`

### 미구현 / 보류 (v0.6 → v0.7 갱신)

- 시퀀스 작성자 **프로필로의 딥링크** — 여전히 미구현 (카드에 작성자 닉네임 표시까지만). 시퀀스 상세로의 딥링크는 이번에 구현
- 옴(좋아요) 알림/집계 대시보드 — 미구현 (카운트 표시만)
- 요가톡 자동 종결/미응답 리마인드 — 미구현 (영구 보존 유지)
...
### 32.2 지도자 (5탭) — v0.7 재구성

- 클래스 (`TeacherClassesTabScreen`) — 상단 **[수업 | 수련생] 세그먼트**로 두 본문 전환 (`ClassesBody` / `MembersBody`, 공용 헤더 공유)
- 아사나 (`AsanasScreen`) — v0.7 추가 (수련생 탭과 동일 화면 재사용)
- 요가톡 (`YogaTalkThreadListScreen`) — 가운데
- 시퀀스 (`TeacherRoutineListScreen`)
- 수련 (`ProfileScreen`)

> v0.6 까지의 "수련생" 독립 탭은 제거되고 클래스 탭 세그먼트로 통합됨. 그 자리에 아사나 탭이 들어와 수련생/지도자 탭이 **클래스 / 아사나 / 요가톡 / 시퀀스 / 수련** 으로 대칭.
